...
from src.GeneTableGenomePoolFileToScfPosBC import GeneTable_Barcodes_To_BarcodeGenes
from src.ScfPosBC_To_BarChartData import ScfPosBC_Info_To_Scaffolds
from src.ScfPosBC_to_MhtnData import PosScfBCDataToZScrPointsForValues
...
def CreateCompleteHTML_dir(genome_fna_fp, genes_table_fp, poolfile_fp, op_dir, genome_name):


    if not os.path.exists(op_dir):
        shutil.copytree("HTML", op_dir)
    else:
    ...
    shutil.move(os.path.join(EBC_data_dir,"EBC_Scaffolds_Init_Data.js"), os.path.join(op_dir, "JS","EBC"))
    ...
    print("Clearing out tmp dir")
    for x in os.listdir("tmp"):
        os.remove(os.path.join("tmp",x))
    print("Opening the file FullDisplay_index.html using a browser (that has Javascript).")
